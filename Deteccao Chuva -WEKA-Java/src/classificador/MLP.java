...
public class MLP {
	...
	private final int QUANTIDADE_EXECUCOES = 1;
	...
	public MLP(float learningRate, float momentum, String hiddenLayers,
			...
			Instances conjuntoTestes, TipoEntrada tipoConjunto) {
		...
		if (validacaoCruzada == false) {
			for (int i = 0; i < QUANTIDADE_EXECUCOES; i++) {
				...
				organizadorResultados.ImprimirEvaluation(pathSaida,
						"Avaliacao Treinamento_MLP_" + i + "_" + tipoConjunto +".txt", evalTreinamento);
				organizadorResultados.ImprimirEvaluation(pathSaida,
						"Avaliacao Teste_MLP_" + i + "_" + tipoConjunto +".txt", evalTeste);
				...
				array_TP_NO_rain_Treinamento[i] = evalTreinamento.truePositiveRate(1);

				organizadorResultados.GerarArquivoClassificacao(
						classificadorMLP, conjuntoTestes, novoConjuntoTeste,
						pathSaida, "resultadoMLP_Testes_" + i + "_" + tipoConjunto +".csv", this.tipoConjunto);
				organizadorResultados.GerarArquivoClassificacao(
						classificadorMLP, conjuntoTreinamento,
						novoConjuntoTreinamento, pathSaida,
						"resultadoMLP_Treinamento_" + i + "_" + tipoConjunto +".csv", this.tipoConjunto);
			}
			DecimalFormat df = new DecimalFormat("0.00");  
			System.out.println("===Treinamento:===");
			System.out.println("TP RAIN: " + df.format(OrganizadorResultados.calcularMedia(array_TP_rain_Treinamento)) + " (" + df.format(OrganizadorResultados.calularDesvioPadrao(array_TP_rain_Treinamento)) + ")");
			System.out.println("TP NO RAIN: " + df.format(OrganizadorResultados.calcularMedia(array_TP_NO_rain_Treinamento))+ " (" + df.format(OrganizadorResultados.calularDesvioPadrao(array_TP_NO_rain_Treinamento)) + ")");
			System.out.println("");
			System.out.println("===Teste:===");
			System.out.println("TP RAIN: " + df.format(OrganizadorResultados.calcularMedia(array_TP_rain)) + " (" + df.format(OrganizadorResultados.calularDesvioPadrao(array_TP_rain)) + ")");
			System.out.println("TP NO RAIN: " + df.format(OrganizadorResultados.calcularMedia(array_TP_NO_rain))+ " (" + df.format(OrganizadorResultados.calularDesvioPadrao(array_TP_NO_rain)) + ")");
		} else {
			// VALIDACAO CRUZADA COM i RUNS e n FOLDS
			ValidacaoCruzada validacao = new ValidacaoCruzada();
			validacao.ExecutarValidacaoCruzada(classificadorMLP,
					novoConjuntoTreinamento, pathSaida,
					"validacaoCruzada_MLP_" + tipoConjunto +".txt", folds, runs);
		}
	}
